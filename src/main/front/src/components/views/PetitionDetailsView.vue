<template>
    <PageStarter
        ...
        :actions="hasSigned? []: [{ label: 'Sign', trigger: signPetition, disabled: !connected }]"
    />
    <TagBadge v-for="tag in petition.properties.tags" :key="tag" :label="tag" :mobile="true" class="md:hidden mt-2"></TagBadge>
    <button v-if="!hasSigned" :disabled="!connected" @click="signPetition()" class="md:hidden w-full mt-6 rounded-md border-0 bg-teal-600 text-slate-200 enabled:focus:ring-2 enabled:focus:ring-teal-400 enabled:hover:bg-teal-500 text-sm font-medium py-1 px-3 disabled:opacity-75 disabled:text-slate-300 disabled:bg-teal-700">Sign this petition</button>

    <div id="petition-content" class="pt-4 md:p-1">
        <p class="text-slate-900 dark:text-slate-200 mb-4 md:font-light">{{ "By " + getDisplayName(petition.properties.author) }}</p>
        ...
        <div id="signatures" class="mb-4">
            <div v-for="signature in signatories.items" :key="signature.key.id" class="flex justify-between items-center mb-2 md:p-3 md:text-sm md:rounded-lg md:bg-teal-50 md:dark:bg-slate-900">
                <div class="text-slate-900 dark:text-slate-200 flex items-center">
                    ...
                    <span class="hidden md:block mr-2">Signed by</span><span>{{ getDisplayName(signature.properties.user) }}</span>
                </div>
...
export default {
    ...
    methods: {
        signPetition() {
            fetch("https://cloudmodule-414213.oa.r.appspot.com/_ah/api/tinyPetApi/ca-marche-sur-mon-pc/signPetition/" + this.petition.key.id,
                { method: "POST", headers: { Authorization: localStorage.getItem("TinyPetGoogleToken") } }
            )
        ...
        getDisplayName(name) {
            ...
            // Name is supposed to look like FirstName_LastName_<EmailAddress>
            // Let's parse that to remove the email adress and replace the underscores by whitespaces
    ...
    mounted() {
        let decodedToken;
        let username;
        this.connected = localStorage.getItem("TinyPetGoogleToken") != null;

        console.log("here!");
        if (this.connected) {
            console.log("I am connected ! I will parse the token!");
            decodedToken = VueJwtDecode.decode(localStorage.getItem("TinyPetGoogleToken"));
            username = decodedToken.given_name + "_" + decodedToken.family_name + "_<" + decodedToken.email + ">";
        } else {
        ...
        fetch("https://cloudmodule-414213.oa.r.appspot.com/_ah/api/tinyPetApi/ca-marche-sur-mon-pc/petitionSignatories/" + this.$route.params.petitionId)
            ...
            .then(data => {
                ...
                this.hasSigned = !this.connected? false: (this.signatories.items.find(sgt => sgt.properties.user == username) != undefined);
            });
    }
}
</script>
